  ...
  if (WiFi.status() == WL_CONNECTED) 
   {
    ...
    Serial.println(NtripSettings.ssid);
...
{ int myIndex;
  ...
  if ( action == ACTION_SET_NTRIPCAST) {
    ...
    if (myIndex >= 0) {
       ...
       EEprom_write_all();
       setAuthorization(NtripSettings.ntripUser, NtripSettings.ntripPassword); // Create new Auth-Code
       client_ntrip.connect(NtripSettings.host, NtripSettings.port);  //reconnect
      }     
   }  
...
void make_HTML01() {
  ...
  strcat( HTML_String, "(Rev. 1.1 by WEder)<br>");
...
// NMEA Sentence
  ...
  strcat( HTML_String, "You can create your own from  www.nmeagen.org<br>");
...
// NMEA transmission
  ...
  for (int i = 0; i < 2; i++) {
    strcat( HTML_String, "<tr>");
    if (i == 0)  strcat( HTML_String, "<td><b>Transmit NMEA over UDP</b></td>");
    else strcat( HTML_String, "<td> </td>");
